...
const FROM = process.env.EMAIL_FROM ?? "Backlink Exchange <no-reply@backlinkexchange.nl>";
...
export async function sendPasswordResetEmail(to: string, resetUrl: string) {
  await sendSafely({
    to,
    subject: "Wachtwoord resetten — Backlink Exchange",
    html: `
...
export async function sendOrderConfirmationEmail(to: string, orderId: string, domain: string, amount: string) {
  ...
  await sendSafely({
    to,
    subject: "Bevestiging van je bestelling — Backlink Exchange",
    html: `
...
export async function sendNewOrderNotificationEmail(to: string, domain: string, amount: string) {
  ...
  await sendSafely({
    to,
    subject: "Nieuwe order ontvangen — Backlink Exchange",
    html: `
